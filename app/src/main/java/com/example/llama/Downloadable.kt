    ...
    companion object {
        ...
        fun Button(viewModel: MainViewModel, dm: DownloadManager, item: Downloadable) {
                ...
                when (val s = status) {
                    ...
                    is Downloading -> {
                        ...
                        val request = DownloadManager.Request(item.source).apply {
                            setTitle("下载模型")
                            setDescription("下载模型: ${item.name}")
                            setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI)
                            setDestinationUri(item.destination.toUri())
                        }

                        viewModel.log("保存 ${item.name} 到 ${item.destination.path}")
                        Log.i(tag, "保存 ${item.name} 到 ${item.destination.path}")
            ...
            Button(onClick = { onClick() }, enabled = status !is Downloading) {
                when (status) {
                    is Downloading -> Text(text = "下载 ${(progress * 100).toInt()}%")
                    is Downloaded -> Text("加载 ${item.name}")
                    is Ready -> Text("下载 ${item.name}")
                    is Error -> Text("下载 ${item.name}")
                }
            }
        }
